...
if(isset($_POST['Submit']))
{
							...
							if($s_pay_option == '1')
								{
									...
									$return= urlencode("https://".$hosted_url."/paypal_listener?uid=".$id."&t=".$DateOfRequest);
	...
	<td><input type="image" src="http://www.paypal.com/en_US/i/btn/x-click-but01.gif" name="submit" title="Make payments with PayPal - it's fast, free and secure!">
									...
									<input type="hidden" name="return" value="https://<?php echo $hosted_url?>/paypal_listener?uid=<?php echo $id;?>&t=<?php echo $DateOfRequest; ?>">
...
$icode = "";
	...
	//	echo $rec['event_name']." ".$rec['option_name']." ".$rec['payment_method']." ".$rec['payment_status'];
		...
		<td colspan="6"><?php if($total > 0){ echo "<a href ='https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=".$user."&lc=US&item_name=".$icode."&amount=".$total."&currency_code=USD&button_subtype=products&no_note=1&no_shipping=1&n=PP%2dBuyNowBF%3abtn_buynowCC_LG%2egif%3aNonHosted&return=".$return."&rm=2'>Click to pay with PayPal : $".$total ."</a>";
	}	?></td>
		</tr>
		<?php
	}
	...
	<form id="eventreg" name ="eventreg" method="post" action="eventreg"  >
    <table border="0" cellpadding="5">
	<tr><td colspan = "2"><div id="reg_list"></div></td></tr>
