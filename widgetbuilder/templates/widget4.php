...
<head>
	...
	<link rel="stylesheet" href="/css/widget4.css" type="text/css">
			...
			<div class="content-inner">
				<div class="chart">
					<div class="channel-one channel">
						<?php
							$total_channel_value = $data['channel_one_value'] + $data['channel_two_value'] + $data['channel_three_value'];
						?>
					...
					<div class="channel-three channel">
						<span class="channel-value"><?php echo $data['channel_three_value']." ".$data['channel_three_unit'];?></span>
						<?php
						$col3_height = ($data['channel_three_value'] / $total_channel_value)*100;
						$var_output = '<span class="channel-column-three channel-column" style="height:'.$col3_height.'px; max-height:85px;"></span>';
						print $var_output;
						?>
						<span class="channel-column-three channel-column"></span>
						<span class="channel-label"><?php echo $data['channel_three_type'];?></span>
					</div>
				</div>
				...
				<div class="target">
					...
					<div class="target-channel">
						<span class="target-channel-label"><?php echo $data['target_three_type'];?>:</span>
						<span class="target-channel-value"><?php echo $data['target_three_value']." ".$data['target_three_unit'];?></span>
					</div>
				</div>
				<?php endif; ?>
			</div>
    		...
    		<div class="content">
    			<div class="content-inner">
    				<div class="chart">
    					...
    					<div class="channel-three channel">
    						<span class="channel-value"><?php 
                             if((isset($_GET['channel_three_value'])) && (trim($_GET['channel_three_value']) != ""))
                             {
                                 if(ereg("([0-9]+)",$_GET['channel_three_value']) && ($_GET['channel_three_value'] < 999)){
                                  $data['channel_three_value'] = htmlspecialchars($_GET['channel_three_value']);
                                  echo $data['channel_three_value']." ".$data['channel_three_unit'];
                                }
                                 else
                                {
                                   echo '<script type="text/javascript">window.alert("Only Numbers < 999 are allowed for channel_three_value")</script>';
    				...
    				<div class="target">
    					...
    					<div class="target-channel">
    						<span class="target-channel-label"><?php echo $data['target_three_type'];?>:</span>
    						<span class="target-channel-value"><?php 
                             if((isset($_GET['target_three_value'])) && (trim($_GET['target_three_value']) != ""))
                             {
                                 if(ereg("([0-9]+)",$_GET['target_three_value'])&& ($_GET['target_three_value'] < 999)){
                                  $data['target_three_value'] = htmlspecialchars($_GET['target_three_value']);
                                  echo $data['target_three_value']." ".$data['target_three_unit'];
                                }
                                 else
                                {
                                   echo '<script type="text/javascript">window.alert("Only Numbers < 999 are allowed for target_one_value")</script>';
